...
public class SniperGunShop : MonoBehaviour
{
    public static SniperGunShop instance;

    [HideInInspector]
    public SniperGun weapon;
    ...
    private void OnEnable()
    {
        ...
        //Check Lock
        LockCover.SetActive(!StageManager.instance.planets[5].PlanetCleared);
        LockText.text = StageManager.instance.planets[5].PlanetName + " 행성 클리어";
    }

    void Update()
    {
        if(weapon.UpgradeLevel > 0)
            WeaponStat.text = "우주선 데미지의 " + ((1 + weapon.ExtraDamagePercent + TraitsManager.instance.GetStats(TraitsData.WeaponType.SniperGun, TraitsData.TraitsType.DamagePercent)) * 100).ToString("F0") + "%로 공격\n" + "현재 데미지: " + (long)Mathf.RoundToInt(Player.instance.FinalAttack_Damage + (weapon.Damage + weapon.ExtraDamage) * (1 + weapon.ExtraDamagePercent + TraitsManager.instance.GetStats(TraitsData.WeaponType.SniperGun, TraitsData.TraitsType.DamagePercent)));
        else
            WeaponStat.text = "적을 타격하는 강력한 저격총";
        ...
        if(StageManager.instance.planets[5].PlanetCleared) {
        ...
        if(GameManager.GetMoney() >= TotalPrice) {
            ...
            for(int i = 0; i < purchaseSize; i++) {
                if(weapon.UpgradeLevel > 0) {
                    ...
                    WeaponManager.instance.sniperGun.gameObject.SetActive(true);
                    weapon.UpgradeWeapon();
                }
            }
